...
import { JsonToHtmlRenderer } from "./JsonToHtmlRenderer";
 ...
 * 
 * Automatically converts candidate search JSON to user-friendly HTML
 */
export function MarkdownRenderer({
  ...
  // Try to render as JSON first (for candidate search results)
  const jsonHtml = JsonToHtmlRenderer({ content });
  if (jsonHtml) {
    return <div className={`markdown-content ${className}`}>{jsonHtml}</div>;
  }

  // Fall back to markdown rendering for regular content
  return (
