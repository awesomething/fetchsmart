...
export function extractDataFromSSE(data: string): ParsedSSEData {
  try {
    ...
    // Handle explicit error payloads from backend (e.g., 429 RESOURCE_EXHAUSTED)
    if (parsed && Object.prototype.hasOwnProperty.call(parsed, "error")) {
      const err = (parsed as unknown as { error?: unknown }).error;
      ...
      // Normalize common 429 message for user clarity
      if (/429/.test(message) || /RESOURCE_EXHAUSTED/i.test(message)) {
        message = `The service is temporarily busy (429: Resource exhausted). Please retry in a few seconds. If this persists, reduce concurrency or switch to the local backend.`;
      }
