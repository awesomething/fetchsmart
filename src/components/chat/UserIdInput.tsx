    ...
    <div className={`${className}`}>
      {isEditing ? (
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          <span className="text-xs text-slate-400 hidden sm:inline">User:</span>
          <Badge
            variant="secondary"
            className="font-mono bg-slate-700/50 text-slate-200 border-slate-600/50 hover:bg-slate-600/50 text-xs px-2 py-0.5"
          >
            {currentUserId}
          </Badge>
          <Button
            ...
            onClick={handleEdit}
            disabled={isLoading}
            className="text-xs h-6 px-1.5 sm:px-2 text-slate-400 hover:text-slate-200 hover:bg-slate-700/50"
          >
            Edit
          </Button>
        </div>
