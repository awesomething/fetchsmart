  ...
  if (!message.content) {
    ...
    return (
      <div className="flex items-start gap-3 max-w-[90%]">
        ...
        // Check if this is an employer progress message
        const isEmployerProgress = message.content.includes("**EMPLOYER_PROGRESS:");
        let progressStage: "transferring" | "creating" | "reviewing" | "analyzing" | "assessing" | "preparing" | "complete" | null = null;
        
        if (isEmployerProgress) {
          const match = message.content.match(/\*\*EMPLOYER_PROGRESS:(\w+)\*\*/);
          if (match) {
            progressStage = match[1] as "transferring" | "creating" | "reviewing" | "analyzing" | "assessing" | "preparing" | "complete";
          }
  }
  ...
  return (
    <div className="flex items-start gap-3 max-w-[90%]">
      ...
      <div className="flex-1 relative group">
        {/* Show progress bar for employer workflow */}
        {isEmployerProgress && progressStage ? (
          <EmployerProgressBar currentStage={progressStage} />
        ) : (
